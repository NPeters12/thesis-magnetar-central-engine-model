...
def line(M, R, min_E, max_E, min_spinP, max_spinP, V_obs, epsilon_r, D_lum_min, D_lum_max, fluxdensitylimit, alpha):
    ...
    B_line_min = np.sqrt(float(fluxdensitylimit)/ (8*10**(7) * V_obs**(-1) * epsilon_r * (10**(dlum_mid))**(-2) * R  * min_spinP**(-4) * 10**3 * (np.sqrt(dlum_mid/1363.5))**(-alpha))) # R is already 10 **6 (in mJy), last is cosmo redshift
    B_line_max = np.sqrt(float(fluxdensitylimit)/ (8*10**(7) * V_obs**(-1) * epsilon_r * (10**(dlum_mid))**(-2) * R  * max_spinP**(-4) * 10**3 * (np.sqrt(dlum_mid/1363.5))**(-alpha) )) # R is already 10 **6 (in mJy), last is cosmo redshift
...
plt.suptitle('Flux density at different field strengths and spin periods of the %s array %.2f mJy limit at %.1f Mhz.' % (L_observatories[obs], fluxdensitylimit, V_obs))
plt.yscale('log')
plt.xscale('log')
...
def plotting(L_alpha):
    ...
    for i in range(Colums):
        ...
        data = test7(M, R, min_E, max_E, min_spinP, max_spinP, V_obs, epsilon_r, D_lum_min, D_lum_max, fluxdensitylimit, L_alpha[i])
        L_P, L_B, L_L, L_T, L_Flux, L_D_lum, L_fdl, visible_percantage, bmesh, pmesh, fmesh = data
        ...
        axs[i].scatter(pmesh, bmesh, c=fmesh, cmap=cm, norm = colors.LogNorm(vmin=min(L_Flux), vmax=max(L_Flux)))
        plt.xlim(10**min_spinP, 10**max_spinP)
        plt.ylim(10**min_E * 10**-15, 10**max_E*10**-15)
# v = np.linspace(min(L_Flux), max(L_Flux), num=10)
        cbar = plt.colorbar()
        ...
        axs[i].set_title('alpha = %.2f, %.1f%% visible' % (L_alpha[i], visible_percantage))
