# sampeling a radio magneter remnent and varieng its efficiency
...
def test7(M, R, min_B, max_B, min_spinP, max_spinP, V_obs, epsilon_r, D_lum_min, D_lum_max, fluxdensitylimit):
    ...
    for n in range(N):
        ...
        # counting amount of flux is higher than the flux density limit
    ...
    return L_P, L_B, L_Flux, L_D_lum, L_fdl, visible_percantage
...
# list of different possible efficiencies (epsilon R)
L_efficiencies = [10**-10, 10**-7, 10**-4, 10**-2]
data = test7(M, R, min_B, max_B, min_spinP, max_spinP, V_obs, epsilon_r, D_lum_min, D_lum_max, fluxdensitylimit)

L_P, L_B, L_Flux, L_D_lum, L_fdl, visible_percantage = data
...
def plotting(L_efficiencies):
    ...
    fig.suptitle('Possible Flux densities at different luminoscity distances with varying efficiencies. With a flux density limit of %.2f mJy at %.1f Mhz.' % (fluxdensitylimit, V_obs), fontsize = 17)
    for i in range(Colums):
        
        data = test7(M, R, min_B, max_B, min_spinP, max_spinP, V_obs, L_efficiencies[i], D_lum_min, D_lum_max, fluxdensitylimit)
        L_P, L_B, L_Flux, L_D_lum, L_fdl, visible_percantage = data
        ...
        axs[i].set_title('eff = %.2E, %.1f%% detectable' % (L_efficiencies[i], visible_percantage))
