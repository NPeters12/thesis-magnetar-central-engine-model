# plotting B-P relation with colormap of flux with redshift encounted (furthest)
...
def test7(M, R, min_E, max_E, min_spinP, max_spinP, V_obs, epsilon_r, D_lum_min, D_lum_max, fluxdensitylimit, alpha):
    ...
    for n in range(N):
        ...
        Flux = 8*10**(7) * V_obs**(-1) * epsilon_r * (10**(D_lum_max))**(-2) * Random_B**2 * R  * Random_P**(-4) * 10**3 * (np.sqrt(D_lum_max/39.231))**(-alpha) # R is already 10 **6 (in mJy), last is cosmo redshift
    ...
    fmesh = (8*10**(7) * V_obs**(-1) * epsilon_r * (10**(D_lum_max))**(-2) * bmesh **2 * R  * pmesh**(-4) * 10**3) * (np.sqrt(D_lum_max/1674.3))**(-alpha)
