...
def test7(M, R, min_E, max_E, min_spinP, max_spinP, V_obs, epsilon_r, D_lum_min, D_lum_max, fluxdensitylimit, alpha):
    ...
    for n in range(N):
        ...
        z = z_at_value(cosmo.luminosity_distance, Random_D * u.Mpc)
        # print z
...
def plotting(L_efficiencies):
    ...
    for i in range(Colums):
        ...
        data = test7(M, R, min_E, max_E, min_spinP, max_spinP, V_obs, L_efficiencies[i], D_lum_min, D_lum_max, fluxdensitylimit, alpha)
        L_P, L_B, L_L, L_T, L_Flux, L_D_lum, L_fdl, visible_percantage = data

        x = L_D_lum
        y = L_Flux
        # Create four polar axes and access them through the returned array
        axs[i].plot(x, y, 'bo', markersize = 0.5)
...
# print z_at_value(cosmo.luminosity_distance, 6701.1 * u.Mpc)
